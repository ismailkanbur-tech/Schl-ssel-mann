    ...
    <main className="container mx-auto px-4 py-12 max-w-6xl">
      ...
      <header className="text-center mb-12">
        <h1 className="text-5xl font-extrabold text-gray-900 mb-4">
          Ihr Ratgeber für Sicherheit und Schlösser
        </h1>
        ...
        {articles.map((article) => (
          <Link key={article.slug} href={`/ratgeber/${article.slug}`} className="block group">
            <div className="bg-white rounded-lg shadow-lg overflow-hidden h-full transform transition duration-300 group-hover:shadow-xl group-hover:scale-[1.02]">
              ...
              {/* ✅ İkon alanı */}
              <div className="h-48 w-full flex items-center justify-center bg-blue-50">
                <article.icon className="h-16 w-16 text-blue-600" />
              </div>
      ...
      <div className="mt-12 text-center border-t pt-8">
        <p className="text-lg text-gray-700">
          Im Notfall? Rufen Sie uns direkt an: <strong>0177 3977646</strong>
        </p>
      </div>
    </main>
  );
