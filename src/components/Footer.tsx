...
import { TruckIcon, PhoneIcon, EnvelopeIcon, MapPinIcon } from '@heroicons/react/24/outline';
...
export default function Footer() {
  ...
  const cities = [
    ...
    { name: "München Pasing", href: "/muenchen/pasing" },
    { name: "Dachau", href: "/dachau" },
    { name: "Markt Indersdorf", href: "/dachau/markt-indersdorf" },
    ...
    <footer className="bg-gray-800 text-gray-300 border-t border-blue-700/50 mt-16">
      <div className="container mx-auto px-4 py-14">
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-10 lg:gap-12">
          ...
          <div className="col-span-2 md:col-span-2">
            ...
            <div className="mt-6 space-y-2 text-sm font-medium">
              ...
              >
                <EnvelopeIcon className="h-4 w-4 mr-3" /> ka-service-dachau@gmx.de
              </a>
            </div>
          </div>
          
          {/* 2. Servicegebiete (ikonlu + dinamik) */}
          <div>
            <h3 className="text-md font-bold text-white uppercase mb-4 tracking-wider">Servicegebiete</h3>
            <ul className="space-y-3 text-sm">
              {cities.map((city) => (
                <li key={city.href} className="flex items-center">
                  <MapPinIcon className="h-4 w-4 text-blue-400 mr-2" />
                  <Link href={city.href} className={`hover:${primaryColor} transition`}>
                    {city.name}
                  </Link>
                </li>
              ))}
        ...
        <div className="mt-12 pt-8 border-t border-gray-700/50 text-center text-xs text-gray-500">
          &copy; {currentYear} KA Service. Alle Rechte vorbehalten. | 
